...
async def list_word_sets(
    ...
    """
    result = await db.execute(select(WordSet).offset(skip).limit(limit))
...
async def get_word_set(
    ...
    """
    result = await db.execute(select(WordSet).where(WordSet.id == set_id))
...
async def create_word_set(
    ...
    result = await db.execute(select(WordSet).where(WordSet.name == word_set.name))
...
async def update_word_set(
    ...
    if word_set_update.name != existing_set.name:
        name_check = await db.execute(select(WordSet).where(WordSet.name == word_set_update.name))
